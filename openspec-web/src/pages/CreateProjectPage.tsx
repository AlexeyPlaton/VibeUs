...
const PLAN_INFO: Record<Plan, { title: string; projects: string }> = {
  free: { title: 'Free', projects: tr('v7.create.plan.free_projects') },
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          ...
          {!authChecked ? (
            ...
            <div className="space-y-6 p-7">
              ...
              <section><div className="mb-2 flex items-center justify-between"><b className="text-sm"><Key className="mr-1 inline h-4 w-4 text-indigo-600" />API Token</b><span className="rounded bg-amber-50 px-2 py-1 text-[11px] font-semibold text-amber-700">SECRET · one-time view</span></div><div className="flex gap-2 rounded-xl border bg-slate-50 p-2"><input readOnly type={showToken ? 'text' : 'password'} value={result.token} className="min-w-0 flex-1 bg-transparent px-2 font-mono text-sm outline-none" /><button onClick={() => setShowToken(!showToken)} className="rounded-lg border bg-white px-3 text-xs">{showToken ? tr('v7.create.success.hide') : tr('v7.create.success.show')}</button><button onClick={() => copy(result.token, 'api')} className="rounded-lg bg-slate-900 px-3 text-xs text-white">{copied === 'api' ? tr('v7.create.common.copied') : tr('v7.create.common.copy')}</button></div></section>

              <section><div className="mb-2 flex items-center justify-between"><b className="text-sm"><Shield className="mr-1 inline h-4 w-4 text-emerald-600" />Public Widget Key</b><span className="rounded bg-emerald-50 px-2 py-1 text-[11px] font-semibold text-emerald-700">{tr('v7.create.success.public_badge')}</span></div><div className="flex gap-2 rounded-xl border bg-slate-50 p-2"><input readOnly value={result.public_widget_key} className="min-w-0 flex-1 bg-transparent px-2 font-mono text-sm outline-none" /><button onClick={() => copy(result.public_widget_key, 'pub')} className="rounded-lg bg-white px-3 text-xs shadow-sm">{copied === 'pub' ? tr('v7.create.common.copied') : tr('v7.create.common.copy')}</button></div></section>

              <section><div className="mb-2 flex items-center justify-between"><b className="text-sm"><Activity className="mr-1 inline h-4 w-4 text-rose-600" />Runtime Ingest Key</b><span className="rounded bg-amber-50 px-2 py-1 text-[11px] font-semibold text-amber-700">SECRET · one-time view</span></div><div className="flex gap-2 rounded-xl border bg-slate-50 p-2"><input readOnly type="password" value={result.ingest_key} className="min-w-0 flex-1 bg-transparent px-2 font-mono text-sm outline-none" /><button onClick={() => copy(result.ingest_key, 'ingest')} className="rounded-lg bg-slate-900 px-3 text-xs text-white">{copied === 'ingest' ? tr('v7.create.common.copied') : tr('v7.create.common.copy')}</button></div><p className="mt-2 text-[11px] text-slate-500">{tr('v7.create.success.ingest_help')}</p></section>
              ...
              <section><b className="text-sm">CLI</b><pre className="mt-2 overflow-x-auto rounded-xl bg-indigo-50 p-4 text-xs text-indigo-900"><Terminal className="mr-1 inline h-4 w-4" />{cliCommand}</pre></section>
            ...
            <form onSubmit={(e) => { e.preventDefault(); continueSubmit().catch((err) => setError(err?.message || tr('v7.create.common.error'))); }} className="space-y-6 p-7">
              ...
              <div><label className="mb-1 block text-sm font-medium">Slug</label><input required minLength={3} value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="frontend-redesign" className="w-full rounded-xl border border-slate-300 px-4 py-3 font-mono text-sm outline-none focus:border-indigo-500" /></div>
