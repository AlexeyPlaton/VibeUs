...
import {
  ArrowLeft, Bot, Check, CheckCircle2, Circle, Copy, ExternalLink, GitBranch,
  RefreshCw, Rocket, Settings2, ShieldCheck, Webhook,
} from 'lucide-react';
...
type AutomationState = {
  ...
  last_check_summary?: Record<string, any>;
};
...
export function AIOrchestrationPage() {
    ...
    try {
      ...
      setSelectedId((current) => {
        const requested = requestedTicket
          ? tickets.find((ticket) => ticket.id === requestedTicket || (ticket.key || '').toLowerCase() === requestedTicket.toLowerCase())
          : null;
  ...
  const missingEvidence = Array.isArray(state?.last_check_summary?.missing_evidence)
    ? state?.last_check_summary?.missing_evidence as string[]
    : [];
  const previewRequest = state?.last_check_summary?.preview_request as Record<string, any> | undefined;
  ...
  const chooseTicket = (ticket: TicketRow) => {
    setSelectedId(ticket.id);
    setHandoff(null);
    setAiAnswer('');
    setPrNumber('');
    const ref = ticket.key || ticket.id;
    navigate(`/app/ai/${encodeURIComponent(projectSlug)}?ticket=${encodeURIComponent(ref)}`, { replace: true });
  };
  ...
  const requestPreview = async () => {
    ...
    setBusy('preview');
    try {
      const data = await request(`/api/projects/${encodeURIComponent(projectSlug)}/tickets/${encodeURIComponent(selected.id)}/automation/preview/deploy`, { method: 'POST', body: '{}' });
      setNotice({ ok: true, text: data.existing ? t('v7.ai_orchestration.preview_already_ready') : t('v7.ai_orchestration.preview_requested') });
      await load();
    } catch (error: any) {
      setNotice({ ok: false, text: error?.message || t('v7.ai_orchestration.preview_request_failed') });
    } finally { setBusy(''); }
  };
  ...
  const hasHandoff = Boolean(state?.base_sha || handoff?.base_sha);
  const hasPr = Boolean(state?.github_pr_number);
  const ciGreen = state?.ci_state === 'success';
  const hasPreview = Boolean(state?.preview_url);
  const reviewReady = selected?.status === 'review' || selected?.status === 'qa' || ['review_ready', 'review_gate_ready', 'preview_ready_for_human_review'].includes(currentStatus);
  const flow = [
    { key: 'task', label: t('v7.ai_orchestration.flow_task'), done: true },
    { key: 'handoff', label: t('v7.ai_orchestration.flow_handoff'), done: hasHandoff },
    { key: 'pr', label: t('v7.ai_orchestration.flow_pr'), done: hasPr },
    { key: 'ci', label: t('v7.ai_orchestration.flow_ci'), done: ciGreen },
    { key: 'preview', label: t('v7.ai_orchestration.flow_preview'), done: hasPreview || reviewReady },
  ];
  const firstWaiting = flow.findIndex((step) => !step.done);
    ...
    <main className="min-h-screen bg-[var(--vb-canvas)] px-4 py-6 text-[var(--vb-text)] sm:px-6">
      <div className="mx-auto max-w-7xl space-y-5">
        <header className="flex flex-col gap-4 border-b border-[var(--vb-border)] pb-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex gap-2"><button type="button" onClick={() => navigate(`/app/integrations/${encodeURIComponent(projectSlug)}`)} className="enterprise-icon-button" aria-label={t('v7.ai_orchestration.open_integrations')}><Settings2 className="h-4 w-4" /></button><button type="button" onClick={() => void load()} className="enterprise-icon-button" aria-label={t('v7.ai_orchestration.refresh')}><RefreshCw className="h-4 w-4" /></button></div>
        </header>

        {!overview.project.github_connected && <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-xs text-amber-200"><span>{t('v7.ai_orchestration.github_required')}</span><button type="button" onClick={() => navigate(`/app/integrations/${encodeURIComponent(projectSlug)}`)} className="rounded-lg border border-amber-400/30 px-3 py-2 font-semibold">{t('v7.ai_orchestration.connect_github')}</button></div>}
        ...
        <section className="rounded-2xl border border-[var(--vb-border)] bg-[var(--vb-surface)] p-4" data-ai-delivery-flow>
          <div className="flex items-center justify-between gap-3"><div><h2 className="text-sm font-semibold">{t('v7.ai_orchestration.flow_title')}</h2><p className="mt-1 text-xs text-[var(--vb-muted)]">{t('v7.ai_orchestration.flow_desc')}</p></div><span className="text-[10px] font-mono text-[var(--vb-muted)]">{selected?.key || selected?.id.slice(0, 8)}</span></div>
          <div className="mt-4 grid gap-2 sm:grid-cols-5">{flow.map((step, index) => { const active = !step.done && (firstWaiting === index); return <div key={step.key} className={`rounded-xl border p-3 text-xs ${step.done ? 'border-emerald-500/25 bg-emerald-500/10' : active ? 'border-[var(--vb-accent)] bg-[var(--vb-accent-soft)]' : 'border-[var(--vb-border)] bg-[var(--vb-canvas-subtle)]'}`} data-ai-flow-step={step.key}><div className="flex items-center gap-2">{step.done ? <CheckCircle2 className="h-4 w-4 text-emerald-400" /> : <Circle className={`h-4 w-4 ${active ? 'text-[var(--vb-accent)]' : 'text-[var(--vb-muted)]'}`} />}<span className="font-semibold">{step.label}</span></div></div>; })}</div>
        </section>

        <section className="grid gap-4 lg:grid-cols-[300px_1fr]">
          <aside className="rounded-2xl border border-[var(--vb-border)] bg-[var(--vb-surface)] p-3"><h2 className="px-1 text-sm font-semibold">{t('v7.ai_orchestration.tickets')}</h2><div className="mt-3 max-h-[760px] space-y-1 overflow-y-auto">{overview.tickets.length === 0 ? <p className="p-3 text-xs text-[var(--vb-muted)]">{t('v7.ai_orchestration.no_tickets')}</p> : overview.tickets.map((ticket) => { const s = ticket.automation?.orchestration_status || 'idle'; return <button key={ticket.id} onClick={() => chooseTicket(ticket)} className={`w-full rounded-xl border p-3 text-left ${selected?.id === ticket.id ? 'border-[var(--vb-accent)] bg-[var(--vb-accent-soft)]' : 'border-transparent hover:border-[var(--vb-border)]'}`}><div className="flex justify-between gap-2"><span className="font-mono text-[10px] text-[var(--vb-accent)]">{ticket.key || ticket.id.slice(0, 8)}</span><span className="text-[10px] text-[var(--vb-muted)]">{ticket.status}</span></div><p className="mt-1 line-clamp-2 text-xs font-semibold">{ticket.title}</p><p className="mt-2 text-[10px] text-[var(--vb-muted)]">{t(statusKey[s] || 'v7.ai_orchestration.status_unknown', { status: s })}</p></button>; })}</div></aside>
          ...
          <div className="space-y-4">
            {selected ? <>
              <section className="rounded-2xl border border-[var(--vb-border)] bg-[var(--vb-surface)] p-4 sm:p-5">
                ...
                {!hasPr && <div className="mt-4 flex flex-wrap gap-2"><button disabled={!overview.project.github_connected || busy === 'handoff'} onClick={() => void generate(false)} className="rounded-lg bg-[var(--vb-accent)] px-3 py-2 text-xs font-semibold text-white disabled:opacity-40"><Bot className="mr-1 inline h-3.5 w-3.5" />{t('v7.ai_orchestration.generate_handoff')}</button>{providerDispatch && <button disabled={!overview.project.github_connected || busy === 'dispatch'} onClick={() => void dispatch()} className="rounded-lg border border-[var(--vb-border)] px-3 py-2 text-xs font-semibold"><Rocket className="mr-1 inline h-3.5 w-3.5" />{t('v7.ai_orchestration.dispatch_agent')}</button>}</div>}
              </section>

              {handoff && !hasPr && <section className="rounded-2xl border border-[var(--vb-border)] bg-[var(--vb-surface)] p-4"><div className="flex items-center justify-between"><h3 className="text-sm font-semibold">{t('v7.ai_orchestration.handoff_prompt')}</h3><button onClick={() => void copy('prompt', handoff.prompt)} className="rounded-lg border border-[var(--vb-border)] px-3 py-1.5 text-xs">{copied === 'prompt' ? <Check className="inline h-3.5 w-3.5" /> : <Copy className="inline h-3.5 w-3.5" />} {t(copied === 'prompt' ? 'v7.ai_orchestration.copied' : 'v7.ai_orchestration.copy_prompt')}</button></div><p className="mt-1 text-xs text-[var(--vb-muted)]">{t('v7.ai_orchestration.handoff_prompt_desc')}</p><pre className="mt-3 max-h-80 overflow-auto whitespace-pre-wrap rounded-xl border border-[var(--vb-border)] bg-[var(--vb-canvas-subtle)] p-3 text-[11px]">{handoff.prompt}</pre></section>}

              {hasHandoff && !hasPr && <section className="rounded-2xl border border-[var(--vb-border)] bg-[var(--vb-surface)] p-4"><h3 className="text-sm font-semibold">{t('v7.ai_orchestration.paste_answer')}</h3><p className="mt-1 text-xs text-[var(--vb-muted)]">{t('v7.ai_orchestration.paste_answer_desc')}</p><textarea rows={8} value={aiAnswer} onChange={(e) => setAiAnswer(e.target.value)} placeholder="VIBEUS-PATCH v1" className="mt-3 w-full rounded-xl border border-[var(--vb-border)] bg-[var(--vb-canvas-subtle)] p-3 font-mono text-xs" /><button disabled={!aiAnswer.trim() || busy === 'patch'} onClick={() => void applyPatch()} className="mt-3 rounded-lg bg-[var(--vb-text)] px-4 py-2 text-xs font-semibold text-[var(--vb-canvas)] disabled:opacity-40"><GitBranch className="mr-1 inline h-3.5 w-3.5" />{t('v7.ai_orchestration.patch_pr')}</button></section>}

              {!hasPr && <section className="rounded-2xl border border-[var(--vb-border)] bg-[var(--vb-surface)] p-4"><h3 className="text-sm font-semibold">{t('v7.ai_orchestration.link_pr')}</h3><p className="mt-1 text-xs text-[var(--vb-muted)]">{t('v7.ai_orchestration.link_pr_desc')}</p><div className="mt-3 flex gap-2"><input type="number" min={1} value={prNumber} onChange={(e) => setPrNumber(e.target.value)} className="w-40 rounded-lg border border-[var(--vb-border)] bg-[var(--vb-canvas-subtle)] px-3 py-2 text-xs" /><button disabled={!prNumber || busy === 'link'} onClick={() => void linkPr()} className="rounded-lg border border-[var(--vb-border)] px-3 py-2 text-xs font-semibold">{t('v7.ai_orchestration.link_pr')}</button></div></section>}

              {hasPr && <section className="rounded-2xl border border-[var(--vb-border)] bg-[var(--vb-surface)] p-4" data-ai-pr-delivery>
                <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="text-sm font-semibold">{t('v7.ai_orchestration.delivery_title')}</h3><p className="mt-1 text-xs text-[var(--vb-muted)]">{t('v7.ai_orchestration.delivery_desc')}</p></div>{state?.github_pr_url && <a href={state.github_pr_url} target="_blank" rel="noreferrer" className="rounded-lg border border-[var(--vb-border)] px-3 py-2 text-xs font-semibold">PR #{state.github_pr_number} <ExternalLink className="ml-1 inline h-3 w-3" /></a>}</div>
                <div className="mt-4 grid gap-2 sm:grid-cols-3"><div className="rounded-lg bg-[var(--vb-canvas-subtle)] p-3 text-xs"><p className="text-[10px] text-[var(--vb-muted)]">{t('v7.ai_orchestration.branch')}</p><p className="mt-1 break-all font-mono">{state?.branch_name || '—'}</p></div><div className="rounded-lg bg-[var(--vb-canvas-subtle)] p-3 text-xs"><p className="text-[10px] text-[var(--vb-muted)]">{t('v7.ai_orchestration.head_sha')}</p><p className="mt-1 font-mono">{shortSha(state?.head_sha)}</p></div><div className="rounded-lg bg-[var(--vb-canvas-subtle)] p-3 text-xs"><p className="text-[10px] text-[var(--vb-muted)]">CI</p><p className="mt-1 font-semibold">{state?.ci_state || 'pending'}</p></div></div>
                {missingEvidence.length > 0 && <div className="mt-3 rounded-xl border border-amber-500/25 bg-amber-500/10 p-3 text-xs text-amber-200"><p className="font-semibold">{t('v7.ai_orchestration.evidence_missing', { count: missingEvidence.length })}</p><ul className="mt-2 list-disc space-y-1 pl-4">{missingEvidence.slice(0, 5).map((item) => <li key={item}>{item}</li>)}</ul></div>}
                {previewRequest && !hasPreview && <div className="mt-3 rounded-xl border border-[var(--vb-border)] bg-[var(--vb-canvas-subtle)] p-3 text-xs"><p className="font-semibold">{t('v7.ai_orchestration.preview_pending')}</p><p className="mt-1 text-[var(--vb-muted)]">{String(previewRequest.provider || '')} · {String(previewRequest.mechanism || '')} · {shortSha(String(previewRequest.head_sha || ''))}</p></div>}
                <div className="mt-4 flex flex-wrap gap-2"><button disabled={busy === 'reconcile'} onClick={() => void reconcile()} className="rounded-lg border border-[var(--vb-border)] px-3 py-2 text-xs font-semibold"><RefreshCw className="mr-1 inline h-3.5 w-3.5" />{t('v7.ai_orchestration.reconcile')}</button>{ciGreen && !hasPreview && <button disabled={busy === 'preview'} onClick={() => void requestPreview()} className="rounded-lg bg-[var(--vb-accent)] px-3 py-2 text-xs font-semibold text-white disabled:opacity-40">{t('v7.ai_orchestration.request_preview')}</button>}{hasPreview && state?.preview_url && <a href={state.preview_url} target="_blank" rel="noreferrer" className="rounded-lg bg-emerald-500/15 px-3 py-2 text-xs font-semibold text-emerald-300">{t('v7.ai_orchestration.open_preview')} <ExternalLink className="ml-1 inline h-3 w-3" /></a>}<button type="button" onClick={() => navigate(`/app/integrations/${encodeURIComponent(projectSlug)}`)} className="rounded-lg border border-[var(--vb-border)] px-3 py-2 text-xs font-semibold">{t('v7.ai_orchestration.open_integrations')}</button>{reviewReady && <button type="button" onClick={() => navigate('/app')} className="rounded-lg bg-emerald-500/15 px-3 py-2 text-xs font-semibold text-emerald-300">{t('v7.ai_orchestration.review_in_board')}</button>}</div>
              </section>}
            </> : null}
          </div>
        </section>

        <details className="rounded-2xl border border-[var(--vb-border)] bg-[var(--vb-surface)] p-4 sm:p-5">
          <summary className="cursor-pointer text-sm font-semibold">{t('v7.ai_orchestration.policy_title')}</summary>
          <p className="mt-1 text-xs text-[var(--vb-muted)]">{t('v7.ai_orchestration.policy_desc')}</p>
          <div className="mt-4 grid gap-3 md:grid-cols-3"><label className="space-y-1 text-xs"><span className="text-[var(--vb-muted)]">{t('v7.ai_orchestration.autonomy')}</span><select value={draft.autonomy_mode} onChange={(e) => setDraft({ ...draft, autonomy_mode: e.target.value as AutonomyMode })} className="w-full rounded-lg border border-[var(--vb-border)] bg-[var(--vb-canvas-subtle)] p-2"><option value="manual">{t('v7.ai_orchestration.mode_manual')}</option><option value="assisted">{t('v7.ai_orchestration.mode_assisted')}</option><option value="autopilot_pr">{t('v7.ai_orchestration.mode_autopilot')}</option><option value="delivery">{t('v7.ai_orchestration.mode_delivery')}</option></select></label><label className="space-y-1 text-xs"><span className="text-[var(--vb-muted)]">{t('v7.ai_orchestration.agent')}</span><select value={draft.agent_kind} onChange={(e) => { const kind = e.target.value as AgentKind; setDraft({ ...draft, agent_kind: kind, dispatch_label: kind === 'jules' ? 'jules' : kind === 'web_ai' ? '' : draft.dispatch_label, auto_dispatch_on_handoff: kind === 'web_ai' ? false : draft.auto_dispatch_on_handoff }); }} className="w-full rounded-lg border border-[var(--vb-border)] bg-[var(--vb-canvas-subtle)] p-2"><option value="web_ai">{t('v7.ai_orchestration.agent_web')}</option><option value="jules">{t('v7.ai_orchestration.agent_jules')}</option><option value="github_label_agent">{t('v7.ai_orchestration.agent_label')}</option><option value="external_agent">{t('v7.ai_orchestration.agent_external')}</option></select></label><label className="space-y-1 text-xs"><span className="text-[var(--vb-muted)]">{t('v7.ai_orchestration.repair_budget')}</span><input type="number" min={0} max={5} value={draft.max_repair_attempts} onChange={(e) => setDraft({ ...draft, max_repair_attempts: Math.max(0, Math.min(5, Number(e.target.value))) })} className="w-full rounded-lg border border-[var(--vb-border)] bg-[var(--vb-canvas-subtle)] p-2" /></label></div>
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{([['auto_issue_sync', 'v7.ai_orchestration.auto_issue'], ['auto_dispatch_on_handoff', 'v7.ai_orchestration.auto_dispatch'], ['create_pr_from_patch', 'v7.ai_orchestration.create_pr'], ['observe_ci', 'v7.ai_orchestration.observe_ci'], ['observe_preview', 'v7.ai_orchestration.observe_preview'], ['auto_move_to_review', 'v7.ai_orchestration.auto_review']] as const).map(([field, label]) => <label key={field} className="flex items-center gap-2 rounded-lg border border-[var(--vb-border)] p-2.5 text-xs"><input type="checkbox" checked={draft[field]} disabled={field === 'auto_dispatch_on_handoff' && !providerDispatch} onChange={(e) => setDraft({ ...draft, [field]: e.target.checked })} /><span>{t(label)}</span></label>)}</div>
          ...
          <button disabled={busy === 'save'} onClick={() => void save()} className="mt-4 rounded-lg bg-[var(--vb-text)] px-4 py-2 text-xs font-semibold text-[var(--vb-canvas)] disabled:opacity-50">{t('v7.ai_orchestration.save_policy')}</button>
        </details>
