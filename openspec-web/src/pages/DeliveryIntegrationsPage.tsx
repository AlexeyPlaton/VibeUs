...
import { ArrowLeft, ExternalLink, GitBranch, RefreshCw, ShieldCheck, Trash2 } from 'lucide-react';
    ...
    try {
      const data = await request(`/api/projects/${encodeURIComponent(projectSlug)}/automation/preview`, {
        ...
        body: JSON.stringify({
          ...
          provider_scope_id: providerScopeId || null,
          review_url: reviewUrl || null,
          api_token: providerToken || null,
      ...
      <div className="mx-auto max-w-5xl space-y-5">
        ...
        <section className="rounded-2xl border border-[var(--vb-border)] bg-[var(--vb-surface)] p-4 sm:p-5" data-github-app-integration>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_auto]">
            ...
            <div className="flex items-end gap-2">
              {github.install_url && <a href={github.install_url} target="_blank" rel="noreferrer" className="rounded-lg border border-[var(--vb-border)] px-3 py-2 text-xs font-semibold">{t('v7.integrations.app_install')} <ExternalLink className="ml-1 inline h-3 w-3" /></a>}
              <button type="button" disabled={!github.configured || !repo.trim() || busy === 'github-connect'} onClick={() => void connectApp()} className="rounded-lg bg-[var(--vb-accent)] px-3 py-2 text-xs font-semibold text-white disabled:opacity-40">{t('v7.integrations.app_verify')}</button>
            </div>
          </div>
        ...
        <section className="rounded-2xl border border-[var(--vb-border)] bg-[var(--vb-surface)] p-4 sm:p-5" data-preview-adapters>
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-2">
            ...
            {(provider === 'vercel' || provider === 'render') && <label className="space-y-1 text-xs"><span className="text-[var(--vb-muted)]">{t('v7.integrations.preview_project_id')}</span><input value={providerProjectId} onChange={(e) => setProviderProjectId(e.target.value)} className="w-full rounded-lg border border-[var(--vb-border)] bg-[var(--vb-canvas-subtle)] px-3 py-2 font-mono" /></label>}
            ...
            {provider === 'render' && <label className="space-y-1 text-xs sm:col-span-2"><span className="text-[var(--vb-muted)]">{t('v7.integrations.preview_review_url')}</span><input value={reviewUrl} onChange={(e) => setReviewUrl(e.target.value)} placeholder="https://review.example.com" className="w-full rounded-lg border border-[var(--vb-border)] bg-[var(--vb-canvas-subtle)] px-3 py-2 font-mono" /><span className="block text-[10px] text-[var(--vb-muted)]">{t('v7.integrations.preview_review_url_help')}</span></label>}
            {(provider === 'vercel' || provider === 'render') && <label className="space-y-1 text-xs sm:col-span-2"><span className="text-[var(--vb-muted)]">{t('v7.integrations.preview_token')}</span><input type="password" autoComplete="off" value={providerToken} onChange={(e) => setProviderToken(e.target.value)} placeholder={t('v7.integrations.preview_token_placeholder')} className="w-full rounded-lg border border-[var(--vb-border)] bg-[var(--vb-canvas-subtle)] px-3 py-2 font-mono" />{preview.has_api_token && <span className="block text-[10px] text-emerald-400">{t('v7.integrations.preview_token_saved')}</span>}</label>}
          </div>
          {preview.has_api_token && (provider === 'vercel' || provider === 'render') && <label className="mt-3 flex items-center gap-2 text-xs"><input type="checkbox" checked={clearToken} onChange={(e) => setClearToken(e.target.checked)} />{t('v7.integrations.preview_clear_token')}</label>}
          <div className="mt-4 rounded-xl border border-[var(--vb-border)] bg-[var(--vb-canvas-subtle)] p-3"><p className="text-xs font-semibold">{t('v7.integrations.observe_only')}</p><p className="mt-1 text-xs text-[var(--vb-muted)]">{t('v7.integrations.observe_only_desc')}</p></div>
