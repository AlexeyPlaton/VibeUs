...
class VibusWidget extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const apiToken = this.getAttribute('api-token') || consumeUrlCapability();
    const publicKey = this.getAttribute('public-key') || this.getAttribute('public-widget-key') || '';
    const rawTheme = this.getAttribute('theme');
    const theme: 'dark' | 'light' | 'auto' = rawTheme === 'light' || rawTheme === 'dark' || rawTheme === 'auto' ? rawTheme : 'dark';
    ...
    // Inject widget CSS stylesheet into Shadow DOM for total style isolation (using self-contained system fonts)
    ...
    container.className = 'vibeus-widget-root vibus-widget-root dark';
