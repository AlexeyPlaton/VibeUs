...
def _resolve_project_root() -> Path:
    ...
    for candidate in candidates:
        ...
        if (candidate / "openspec-core").is_dir() and (candidate / "openspec-web").is_dir():
            return candidate
    pytest.fail("VIBUS project root not found. Set VIBUS_PROJECT_ROOT to repository root.")
...
def service_modules(core_dir: Path):
    ...
    os.environ.setdefault("TOKEN_PEPPER", "qgv5-token-pepper-0123456789abcdef0123456789")
    os.environ.setdefault("FIELD_ENCRYPTION_KEY", "qgv5-field-key-0123456789abcdef0123456789")
    ...
    return SimpleNamespace(models=models, schemas=schemas, yookassa_service=yookassa_service)
...
def backend_runtime(core_dir: Path, tmp_path_factory):
    db_file = tmp_path_factory.mktemp("qgv5") / "runtime.sqlite"
    os.environ["DATABASE_URL"] = f"sqlite+aiosqlite:///{db_file.as_posix()}"
    os.environ["ENVIRONMENT"] = "test"
    os.environ["TESTING"] = "true"
    os.environ["TOKEN_PEPPER"] = "qgv5-runtime-pepper-0123456789abcdef0123456789"
    os.environ["FIELD_ENCRYPTION_KEY"] = "qgv5-runtime-field-0123456789abcdef0123456789"
    ...
    try:
        ...
        manage_receipts = importlib.import_module("manage_receipts")
    except Exception as exc:
        pytest.fail(
            "Could not import openspec-core. Install runtime dependencies first. "
...
async def api_client(fresh_backend):
    ...
    async with AsyncClient(transport=ASGITransport(app=fresh_backend.app), base_url="http://qgv5.local") as client:
        yield client
